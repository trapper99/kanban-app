...
function App() {
  ...
  const addNewCard = useCallback((listId) => {
  ...
  const onCardStop = useCallback((e, card, listId) => {
    ...
    <div>
      ...
      <div style={styles.boardContainer}>
        {memoizedBoard.map((list) => (
          <div id={`list_${list.id}`} key={list.id} className="list-container" className={styles.listContainer}>
            <h2>{list.title}</h2>
